...
import { clonePlanByToken } from "@/lib/clone-actions";
...
export default async function ClonePage({
  ...
  if (!user) {
    redirect(`/login?next=${encodeURIComponent(`/clone/${token}`)}`);
  }

  const result = await clonePlanByToken(token);

  if ("newPlanId" in result) {
    redirect(`/plans/${result.newPlanId}`);
  }
